...
$out=Join-Path $env:RUNNER_TEMP 'windows-servercore-powershell.json'
$state=[ordered]@{
  schema='windows-servercore-powershell/v1'
...
function Run-Probe([string]$isolation){
  ...
  $ps = '$g=[guid]::NewGuid().ToString(); Write-Output ("CONTAINER_HOST="+$env:COMPUTERNAME); Write-Output ("NONCE_PREFIX='+$prefix+'"); Write-Output ("GUID_NONCE="+$g); Write-Output ("PS_VERSION="+$PSVersionTable.PSVersion.ToString())'
  $sw=[Diagnostics.Stopwatch]::StartNew()
  $text=(& docker run --rm --isolation=$isolation $image powershell.exe -NoLogo -NoProfile -NonInteractive -Command $ps 2>&1|Out-String).Trim()
  $code=$LASTEXITCODE
  $sw.Stop()
  [ordered]@{
    attempted=$true
    ...
    output=$text
    oracleSatisfied=($code -eq 0 -and $text -match [regex]::Escape("NONCE_PREFIX=$prefix") -and $text -match 'GUID_NONCE=[0-9a-fA-F-]{36}' -and $text -match 'PS_VERSION=')
  }
}
...
try {
  ...
  if($free -lt 10GB){throw 'Less than 10 GiB free; refusing Server Core experiment'}
  ...
  $state.classification=if($state.hyperv.oracleSatisfied){
    'WINDOWS_SERVERCORE_POWERSHELL_HYPERV_ISOLATION_PROVEN'
  }elseif($state.process.oracleSatisfied){
    'WINDOWS_SERVERCORE_POWERSHELL_PROCESS_ISOLATION_ONLY'
  }else{
    'WINDOWS_SERVERCORE_POWERSHELL_EXECUTION_UNPROVEN'
  }
} catch {
