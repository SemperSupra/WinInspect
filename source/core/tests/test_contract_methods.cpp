...
DOCTEST_TEST_CASE("contract: window.setProperty")
{
  ...
  json::Object p;
  p["hwnd"] = std::string("0x1");
  p["name"] = std::string("topmost");
  p["value"] = std::string("true");
...
DOCTEST_TEST_CASE("contract: window.findRegex")
{
  ...
  CoreEngine core(&fb);
  json::Object p;
  p["title_regex"] = std::string(".*");
  p["class_regex"] = std::string(".*");
...
DOCTEST_TEST_CASE("contract: clipboard.write")
{
  ...
  CoreEngine core(&fb);
  json::Object p;
  p["text"] = std::string("contract test");
...
DOCTEST_TEST_CASE("contract: env.set")
{
  ...
  CoreEngine core(&fb);
  json::Object p;
  p["name"] = std::string("TEST_VAR");
  p["value"] = std::string("test_value");
...
DOCTEST_TEST_CASE("contract: reg.read")
{
  ...
  CoreEngine core(&fb);
  json::Object p;
  p["path"] = std::string("HKCU\\Software\\Test");
...
DOCTEST_TEST_CASE("contract: reg.write")
{
  ...
  json::Object p;
  p["path"] = std::string("HKCU\\Software\\Test");
  p["name"] = std::string("TestVal");
  p["type"] = std::string("SZ");
  p["data"] = std::string("hello");
...
DOCTEST_TEST_CASE("contract: daemon.notify")
{
  auto fb = make_fake();
  CoreEngine core(&fb);
  json::Object p;
  p["title"] = std::string("Test Notification");
  p["message"] = std::string("Agent is about to click the mouse");
  CoreRequest req{"t43", "daemon.notify", p};
  auto r = core.handle(req, fb.capture_snapshot());
  DOCTEST_REQUIRE(r.ok);
}
...
DOCTEST_TEST_CASE("contract: ui.invoke")
{
  ...
  json::Object p;
  p["hwnd"] = std::string("0x1");
  p["automation_id"] = std::string("btn");
...
DOCTEST_TEST_CASE("contract: empty window list returns empty")
{
  auto fb = make_fake();
  fb.set_empty_windows(true);
  CoreEngine core(&fb);
  auto snap = fb.capture_snapshot();
  DOCTEST_REQUIRE(snap.top.empty());
  CoreRequest req{"err4", "window.listTop", {}};
  auto r = core.handle(req, snap);
  DOCTEST_REQUIRE(r.ok);
  DOCTEST_REQUIRE(r.result.is_arr());
  DOCTEST_REQUIRE(r.result.as_arr().empty());
}

DOCTEST_TEST_CASE("contract: error injection causes failure")
{
  auto fb = make_fake();
  fb.set_fail_next("window.listTop", true);
  CoreEngine core(&fb);
  CoreRequest req{"err5", "window.listTop", {}};
  auto r = core.handle(req, fb.capture_snapshot());
  DOCTEST_REQUIRE(r.ok); // empty result is "ok"
  DOCTEST_REQUIRE(r.result.is_arr());
  DOCTEST_REQUIRE(r.result.as_arr().empty());
}

DOCTEST_TEST_CASE("contract: null results returns no info")
{
  auto fb = make_fake();
  fb.set_null_results(true);
  CoreEngine core(&fb);
  json::Object p;
  p["hwnd"] = std::string("0x1");
  CoreRequest req{"err6", "window.getInfo", p};
  auto r = core.handle(req, fb.capture_snapshot());
  DOCTEST_REQUIRE(!r.ok);
  DOCTEST_REQUIRE(r.error_code == "E_BAD_HWND");
}

DOCTEST_TEST_CASE("contract: custom capabilities override")
{
  auto fb = make_fake();
  Capabilities caps;
  caps.os = "custom os";
  caps.uia_available = false;
  caps.input_injection = true;
  fb.set_custom_capabilities(caps);
  auto result = fb.get_capabilities();
  DOCTEST_REQUIRE(result.os == "custom os");
  DOCTEST_REQUIRE(result.uia_available == false);
  DOCTEST_REQUIRE(result.input_injection == true);
}
...
DOCTEST_TEST_CASE("contract: bad method returns E_BAD_METHOD")
{
  auto fb = make_fake();
  CoreEngine core(&fb);
  CoreRequest req{"err_bad", "nonexistent.method", {}};
  auto r = core.handle(req, fb.capture_snapshot());
  DOCTEST_REQUIRE(!r.ok);
  DOCTEST_REQUIRE_EQ(r.error_code, "E_BAD_METHOD");
}

DOCTEST_TEST_CASE("contract: missing capability blocks method")
{
  auto fb = make_fake();
  Capabilities caps;
  caps.input_injection = false; // deny input capability
  caps.clipboard_available = true;
  fb.set_custom_capabilities(caps);

  CoreEngine core(&fb);

  // Input method blocked by missing capability
  CoreRequest req_input{"cap1", "input.send", {}};
  auto r1 = core.handle(req_input, fb.capture_snapshot());
  DOCTEST_REQUIRE(!r1.ok);
  DOCTEST_REQUIRE_EQ(r1.error_code, "E_ACCESS_DENIED");

  // Clipboard method still works (capability available)
  json::Object p;
  p["text"] = "test";
  CoreRequest req_clip{"cap2", "clipboard.write", p};
  auto r2 = core.handle(req_clip, fb.capture_snapshot());
  DOCTEST_REQUIRE(r2.ok);

  // Method with no capability requirement is unaffected
  CoreRequest req_list{"cap3", "window.listTop", {}};
  auto r3 = core.handle(req_list, fb.capture_snapshot());
  DOCTEST_REQUIRE(r3.ok);
}

DOCTEST_TEST_CASE("contract: available capability allows method")
{
  auto fb = make_fake();
  Capabilities caps;
  caps.input_injection = true;
  ...
  // Clipboard read requires no params and is tagged with "clipboard" capability
  CoreRequest req_clip{"cap_ok1", "clipboard.read", {}};
  auto r1 = core.handle(req_clip, fb.capture_snapshot());
  DOCTEST_REQUIRE(r1.ok);

  // Window list requires no capability tag at all
  CoreRequest req_list{"cap_ok2", "window.listTop", {}};
  auto r2 = core.handle(req_list, fb.capture_snapshot());
  DOCTEST_REQUIRE(r2.ok);
}
