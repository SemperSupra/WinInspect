...
static const char* kTwoClientsNonInterferenceTrace = R"json({
  "name": "two_clients_non_interference",
  "trace_version": "1",
  "initial_world": {
    "windows": [
      { "hwnd": "0x1", "visible": true,  "title": "A", "class": "C1" },
      { "hwnd": "0x2", "visible": false, "title": "B", "class": "C2" }
    ]
  },
  "steps": [
    { "client": "c1", "send": { "id": "c1-1", "method": "window.getInfo", "params": { "snapshot_id": "s-fixed", "hwnd": "0x1", "canonical": true } } },
    { "client": "c2", "send": { "id": "c2-1", "method": "window.ensureVisible", "params": { "hwnd": "0x2", "visible": true, "canonical": true } } },
    { "client": "c1", "expect_response": { "id": "c1-1", "ok": true } },
    { "client": "c2", "expect_response": { "id": "c2-1", "ok": true } },
    { "assert": { "type": "non_interference" } },
    { "client": "c2", "send": { "id": "c2-2", "method": "window.ensureVisible", "params": { "hwnd": "0x2", "visible": true, "canonical": true } } },
    { "client": "c2", "expect_response": { "id": "c2-2", "ok": true, "result": { "changed": false } } }
  ]
})json";

static const char* kUiaRecursiveTreeTrace = R"json({
  "name": "UIA Recursive Tree Discovery",
  "trace_version": "1",
  "description": "Canonical trace for a nested UI tree (Window -> Panel -> Button)",
  "initial_world": {
    "windows": [
      {
        "hwnd": "0x1001",
  ...
  "steps": [
    {
      ...
      "expected_result": [
        {
          "automation_id": "root_pane",
          "name": "Root Pane",
          "control_type": "50033",
          "bounding_rect": { "left": 0, "top": 0, "right": 800, "bottom": 600 },
          "enabled": true,
          "visible": true,
          "children": [
            {
              ...
              "name": "Submit",
              "control_type": "50000",
              "bounding_rect": { "left": 100, "top": 100, "right": 200, "bottom": 130 },
              "enabled": true,
              "visible": true,
              "children": []
            }
...
// Fails the test with a clear message if the version is missing or unsupported.
static void require_trace_version(const char* path, const json::Object& trace)
{
  auto it = trace.find("trace_version");
  // trace_version must exist
  DOCTEST_REQUIRE(it != trace.end());
  // trace_version must be a string
  DOCTEST_REQUIRE(it->second.is_str());
  auto ver = it->second.as_str();
  // only version "1" is supported
  DOCTEST_REQUIRE(ver == "1");
}

DOCTEST_TEST_CASE("trace replay: two_clients_non_interference")
{
  // Minimal replay: validate key expectations using fake backend
  auto trace = wininspect::json::parse(read_trace("formal/traces/two_clients_non_interference.json",
                                                  kTwoClientsNonInterferenceTrace))
                   .as_obj();
  ...
  for (const auto& w : windows) {
    ...
    fw.push_back({(hwnd_u64)hwnd, 0, 0, o.at("title").as_str(), o.at("class").as_str(),
                  o.at("visible").as_bool()});
  }
...
{
  auto trace = wininspect::json::parse(read_trace("formal/traces/uia_recursive_tree.json",
                                                  kUiaRecursiveTreeTrace)).as_obj();
  ...
  DOCTEST_REQUIRE(res.at("children").as_arr()[0].as_obj().at("automation_id").as_str() ==
                  "submit_btn");
}
...
DOCTEST_TEST_CASE("trace replay: system_orchestration")
{
  ...
  // Registry Read
  json::Object p2;
  p2["path"] = std::string("HKCU\\Software\\Test");
