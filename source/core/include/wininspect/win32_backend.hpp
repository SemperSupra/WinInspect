...
namespace wininspect {
  ...
  public:
    ...
    bool start_recording(const std::string&, int, int, bool) override { return false; }
    bool stop_recording() override { return false; }

  private:
    bool is_wine_ = false;
    uint32_t win_major_ = 0;
    long last_cursor_x_ = 0;
    long last_cursor_y_ = 0;
    ...
    // WARP D3D11 software device capture (Windows only, no Driver Type HARDWARE needed)
    std::optional<ScreenCapture> try_warp_capture(Rect region);

    // GDI capture with timeout — prevents indefinite hangs on locked/disconnected sessions
    std::optional<ScreenCapture> gdi_capture_impl(Rect region);
    std::optional<ScreenCapture> gdi_capture_with_timeout(Rect region, int timeout_ms = 5000);
  };
