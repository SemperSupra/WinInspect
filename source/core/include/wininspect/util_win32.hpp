...
namespace wininspect {
  ...
  {
    HRESULT hr;
    CoInitGuard(DWORD dwCoInit = COINIT_MULTITHREADED) { hr = CoInitializeEx(NULL, dwCoInit); }
    ~CoInitGuard()
    {
      if (SUCCEEDED(hr))
        CoUninitialize();
  ...
  public:
    ...
    ~HKey()
    {
      if (h_ != NULL && h_ != HKEY_LOCAL_MACHINE && h_ != HKEY_CURRENT_USER &&
          h_ != HKEY_CLASSES_ROOT && h_ != HKEY_USERS)
        RegCloseKey(h_);
    }
