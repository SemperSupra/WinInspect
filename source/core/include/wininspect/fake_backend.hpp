...
namespace wininspect {
  ...
  public:
    ...
    std::optional<std::pair<int, int>> pixel_search(Rect region, Color target,
    ...
    // Advanced Discovery
    std::vector<hwnd_u64> find_windows_regex(const std::string& title_regex,
                                             const std::string& class_regex) override;
    ...
    std::optional<ImageMatchResult> image_match(Rect region,
                                                const std::vector<uint8_t>& sub_image_bmp) override;
    ...
    // Error injection — cause specific methods to fail for testing error paths
    void set_fail_next(const std::string& method, bool should_fail = true);
    bool fail_next(const std::string& method) const;
    void set_empty_windows(bool empty) { empty_windows_ = empty; }
    void set_null_results(bool null) { null_results_ = null; }
    void set_custom_capabilities(const Capabilities& caps) { custom_caps_ = caps; }

    std::vector<Event> poll_events(const Snapshot& old_snap, const Snapshot& new_snap) override;
    bool poll_local_input() override;

    bool start_recording(const std::string&, int, int, bool) override { return true; }
    bool stop_recording() override { return true; }

  private:
    mutable std::mutex mu_;
    std::map<hwnd_u64, FakeWindow> w_;
    hwnd_u64 foreground_ = 0;
    bool empty_windows_ = false;
    bool null_results_ = false;
    // fail_next_ set is accessed with its own mutex to avoid double-lock
    // when checked from methods that already hold mu_
    mutable std::mutex fail_mu_;
    std::set<std::string> fail_next_;
    std::optional<Capabilities> custom_caps_;
