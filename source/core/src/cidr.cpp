...
namespace wininspect {
  ...
  bool parse_cidr(const std::string& cidr_str, struct sockaddr_storage& net, int& prefix_len)
  {
  ...
  bool cidr_match(const std::string& addr_str, const std::string& cidr_str)
  {
    ...
    int prefix_len = 0;
    if (!parse_cidr(cidr_str, net, prefix_len))
      return false;
  ...
  bool cidr_match_any(const std::string& addr_str, const std::vector<std::string>& cidr_list)
  {
    for (auto& cidr : cidr_list) {
      if (cidr_match(addr_str, cidr))
        return true;
    }
    return false;
  }
