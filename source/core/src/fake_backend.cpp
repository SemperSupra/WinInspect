...
namespace wininspect {
  ...
  void FakeBackend::set_fail_next(const std::string& method, bool should_fail)
  {
    std::lock_guard<std::mutex> lk(fail_mu_);
    if (should_fail)
      fail_next_.insert(method);
    else
      fail_next_.erase(method);
  }

  bool FakeBackend::fail_next(const std::string& method) const
  {
    std::lock_guard<std::mutex> lk(fail_mu_);
    return fail_next_.count(method) > 0;
  }
  ...
  std::vector<hwnd_u64> FakeBackend::list_top(const Snapshot& s)
  {
    if (fail_next("window.listTop"))
      return {};
    std::lock_guard<std::mutex> lk(mu_);
    return s.top;
  }

  std::vector<hwnd_u64> FakeBackend::list_children(const Snapshot&, hwnd_u64 parent)
  {
    std::lock_guard<std::mutex> lk(mu_);
  ...
  std::optional<WindowInfo> FakeBackend::get_info(const Snapshot&, hwnd_u64 hwnd)
  {
    if (fail_next("window.getInfo"))
      return std::nullopt;
    std::lock_guard<std::mutex> lk(mu_);
    if (null_results_)
      return std::nullopt;
  ...
  std::optional<hwnd_u64> FakeBackend::pick_at_point(const Snapshot&, int, int, PickFlags)
  {
  ...
  std::vector<WindowNode> FakeBackend::get_window_tree(const Snapshot&, hwnd_u64 root_u)
  {
    std::lock_guard<std::mutex> lk(mu_);
    ...
    if (root_u == 0) {
      for (const auto& [h, w] : w_) {
        if (w.parent == 0)
          results.push_back(build_node(build_node, h));
      }
    }
    else {
  ...
  bool FakeBackend::set_property(hwnd_u64 hwnd, const std::string& name, const std::string& value)
  {
    std::lock_guard<std::mutex> lk(mu_);
  ...
  std::vector<hwnd_u64> FakeBackend::find_windows_regex(const std::string&, const std::string&)
  {
    return {0x1234};
  }
  ...
  std::optional<std::string> FakeBackend::clipboard_read()
  {
  ...
  std::vector<ServiceInfo> FakeBackend::service_list()
  {
    return {{"FakeSvc", "Fake Service", "RUNNING"}};
  }
  std::string FakeBackend::service_status(const std::string&)
  {
    return "RUNNING";
  }
  ...
  std::vector<EnvVar> FakeBackend::env_get_all()
  {
    return {{"PATH", "C:\\fake"}};
  }
  ...
  std::vector<DriveInfo> FakeBackend::wine_get_drives()
  {
    return {{"C", "C:\\", "Fixed"}};
  }
  std::vector<std::string> FakeBackend::wine_get_overrides()
  {
    return {"d3d11=native"};
  }

  bool FakeBackend::sync_check_mutex(const std::string&)
  {
    return true;
  }
  ...
  bool FakeBackend::poll_local_input()
  {
    // FakeBackend returns false — no local input in tests unless explicitly set
    return false;
  }
  ...
  {
    std::lock_guard<std::mutex> lk(mu_);
    injected_events_.push_back("mouse_click:" + std::to_string(x) + "," + std::to_string(y) + "," +
  ...
  static bool find_and_invoke(const std::vector<UIElementInfo>& elements, const std::string& id)
  {
  ...
  bool FakeBackend::invoke_ui_element(hwnd_u64 hwnd, const std::string& automation_id)
  {
    std::lock_guard<std::mutex> lk(mu_);
  ...
  std::vector<Event> FakeBackend::poll_events(const Snapshot&, const Snapshot&)
  {
    return {}; // Placeholder
  }
