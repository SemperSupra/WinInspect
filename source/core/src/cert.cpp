...
namespace wininspect {
  ...
  std::string cert_fields_to_string(const Certificate& cert)
  {
    ...
    ss << cert.uuid << "|" << cert.instance_name << "|" << cert.hostname << "|" << cert.public_key
       << "|" << cert.ca_fingerprint << "|" << cert.issued_at << "|" << cert.expires_at;
    return ss.str();
  }
  ...
  std::string generate_ca_key()
  {
    uint8_t key[32];
#ifdef _WIN32
    if (BCryptGenRandom(nullptr, key, sizeof(key), BCRYPT_USE_SYSTEM_PREFERRED_RNG) < 0) {
  ...
  Certificate sign_certificate(const std::string& ca_key, const std::string& uuid,
                               const std::string& instance_name, const std::string& hostname,
                               const std::string& ecdh_pubkey, int ttl_days)
  {
    Certificate cert;
    cert.uuid = uuid;
    ...
    auto sig = hmac_sha256(ca_key_raw, std::vector<uint8_t>(fields.begin(), fields.end()));
  ...
  bool verify_certificate(const Certificate& cert, const std::string& ca_key)
  {
    ...
    // Verify signature
    auto fields = cert_fields_to_string(cert);
    auto expected_sig = hmac_sha256(ca_key_raw, std::vector<uint8_t>(fields.begin(), fields.end()));
  ...
  Certificate Certificate::from_json(const json::Object& o)
  {
    ...
    if (it != o.end() && it->second.is_str())
      cert.uuid = it->second.as_str();
    it = o.find("instance_name");
    if (it != o.end() && it->second.is_str())
      cert.instance_name = it->second.as_str();
    it = o.find("hostname");
    if (it != o.end() && it->second.is_str())
      cert.hostname = it->second.as_str();
    it = o.find("public_key");
    if (it != o.end() && it->second.is_str())
      cert.public_key = it->second.as_str();
    it = o.find("ca_fingerprint");
    if (it != o.end() && it->second.is_str())
      cert.ca_fingerprint = it->second.as_str();
    it = o.find("issued_at");
    if (it != o.end() && it->second.is_num())
      cert.issued_at = (int64_t)it->second.as_num();
    it = o.find("expires_at");
    if (it != o.end() && it->second.is_num())
      cert.expires_at = (int64_t)it->second.as_num();
    it = o.find("signature");
    if (it != o.end() && it->second.is_str())
      cert.signature = it->second.as_str();
    return cert;
  }
