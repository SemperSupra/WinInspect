...
namespace wininspect::update {
  ...
  static std::string https_get(const std::wstring& host, const std::wstring& path)
  {
    ...
    HINTERNET hSession = WinHttpOpen(L"WinInspect/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
    ...
    BOOL ok = WinHttpSendRequest(hRequest, headers, (DWORD)wcslen(headers), WINHTTP_NO_REQUEST_DATA,
                                 0, 0, 0);
  ...
  UpdateInfo check_for_update(const std::string& current_version)
  {
    ...
    std::string json_body =
        https_get(L"api.github.com", L"/repos/SemperSupra/WinInspect/releases/latest");
    if (json_body.empty()) {
    ...
    try {
      ...
      if (it_assets != obj.end() && it_assets->second.is_arr()) {
        for (const auto& asset : it_assets->second.as_arr()) {
          ...
          if (it_name == a.end() || it_dl == a.end())
            continue;
          if (!it_name->second.is_str() || !it_dl->second.is_str())
            continue;
          ...
          else if (name.find("Portable") != std::string::npos &&
                   name.find(".zip") != std::string::npos && info.portable_zip_url.empty())
  ...
  static bool download_to_file(const std::string& url, const std::wstring& outPath)
  {
    ...
    HINTERNET hRequest =
        WinHttpOpenRequest(hConnect, L"GET", wpath.c_str(), nullptr, WINHTTP_NO_REFERER,
                           WINHTTP_DEFAULT_ACCEPT_TYPES, WINHTTP_FLAG_SECURE);
    if (!hRequest) {
      WinHttpCloseHandle(hConnect);
      WinHttpCloseHandle(hSession);
    ...
    HANDLE hFile = CreateFileW(outPath.c_str(), GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS,
                              ...
                              const std::string& type_hint)
  {
    ...
    if (!download_to_file(url, outFile))
      return {};
  ...
  bool verify_checksum(const std::string& file_path, const std::string& expected_hex)
  {
    ...
    if (hFile == INVALID_HANDLE_VALUE) {
      LOG_ERROR("Cannot open file for checksum: " + file_path);
      return false;
    }
    ...
    if (computed != expected_hex) {
      LOG_ERROR("Checksum mismatch: computed=" + computed + " expected=" + expected_hex);
      return false;
    }
  ...
  bool apply_update(const std::string& installer_path, const std::string& restart_command)
  {
    // Build the silent install command: NSIS /S = silent, /D sets install directory
    // The daemon is already running from Program Files, so we install to the same place
    wchar_t exePath[MAX_PATH];
    GetModuleFileNameW(nullptr, exePath, MAX_PATH);
    std::wstring exeDir(exePath);
    size_t lastBS = exeDir.rfind(L'\\');
    if (lastBS != std::wstring::npos)
      exeDir.resize(lastBS);

    std::wstring wPath(installer_path.begin(), installer_path.end());
    std::wstring cmd = L"\"" + wPath + L"\" /S /D=" + exeDir;

    LOG_INFO("Applying update: " + installer_path);
    ...
    if (!CreateProcessW(nullptr, &cmd[0], nullptr, nullptr, FALSE, CREATE_NO_WINDOW, nullptr,
                        nullptr, &si, &pi)) {
      LOG_ERROR("Failed to start installer: " + std::to_string(GetLastError()));
      return false;
    }

    // Wait for installer to complete (up to 2 minutes)
    DWORD waitResult = WaitForSingleObject(pi.hProcess, 120000);
    CloseHandle(pi.hProcess);
    CloseHandle(pi.hThread);

    if (waitResult != WAIT_OBJECT_0) {
      LOG_ERROR("Installer did not complete within 2 minutes");
      return false;
    }

    LOG_INFO("Update installed successfully");

    // Restart the daemon if a restart command is provided
    if (!restart_command.empty()) {
      LOG_INFO("Restarting daemon: " + restart_command);
      std::wstring wRestart(restart_command.begin(), restart_command.end());
      STARTUPINFOW si2 = {};
      si2.cb = sizeof(si2);
      PROCESS_INFORMATION pi2 = {};
      if (CreateProcessW(nullptr, &wRestart[0], nullptr, nullptr, FALSE, CREATE_NO_WINDOW, nullptr,
                         nullptr, &si2, &pi2)) {
