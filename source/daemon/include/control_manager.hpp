...
namespace wininspectd {
  ...
  public:
    ...
    [[nodiscard]] bool take_control(wininspect::ControllerType who, const std::string& id = "");

    /// Release control. Only the current controller can release (except human).
    [[nodiscard]] bool release_control(wininspect::ControllerType who, const std::string& id = "");

    /// Get current control state as JSON.
    wininspect::json::Object get_status() const;

    /// Log an action to the audit trail.
    void log_action(const std::string& method, const wininspect::json::Object& params, bool ok,
                    int64_t duration_ms);
    ...
    void set_max_entries(size_t max) { max_audit_entries_ = max; }

    /// Check if an agent/script can perform input injection.
    [[nodiscard]] bool can_inject_input() const;

    /// Notify that local input was detected (human took over).
    void notify_local_input();

    /// Get current controller type (None, Human, Agent, Script).
    wininspect::ControllerType current_controller() const;

    /// Persist audit log to a JSON-lines file.
    void save_audit_log(const std::string& path);

    /// Load audit log from a JSON-lines file.
    void load_audit_log(const std::string& path);
