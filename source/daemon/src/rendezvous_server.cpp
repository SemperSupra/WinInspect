...
namespace {
  ...
  std::string g_auth_token; // set via --auth-token, empty = no auth
  ...
  /// Check Authorization header against configured auth token.
  /// Returns true if auth is not configured (empty token) or token matches.
  static bool check_auth(const HttpRequest& req, HttpResponse& resp)
  {
    ...
    if (it == req.headers.end() || it->second.empty()) {
      ...
      resp.body = R"({"error":"missing authorization header"})";
      return false;
    }
    auto& val = it->second;
    // Bearer token comparison
    if (val.size() > 7 && val.substr(0, 7) == "Bearer " && val.substr(7) == g_auth_token)
      return true;
    // Rendezvous HMAC comparison (legacy client support)
    if (val.size() > 10 && val.substr(0, 10) == "Rendezvous " && val.substr(10) == g_auth_token)
      return true;
    resp.status_code = 401;
    resp.status_text = "Unauthorized";
    resp.body = R"({"error":"invalid authorization token"})";
    return false;
  }

  static void handle_register(const HttpRequest& req, HttpResponse& resp)
  {
    if (!check_auth(req, resp))
      return;
    try {
      auto v = json::parse(req.body);
      if (!v.is_obj()) {
        resp.status_code = 400;
        resp.body = R"({"error":"expected object"})";
        return;
      }
      auto obj = v.as_obj();
      ...
      if (it_uuid == obj.end() || !it_uuid->second.is_str() || it_uuid->second.as_str().empty()) {
        resp.status_code = 400;
        resp.body = R"({"error":"missing uuid"})";
        return;
      }
      ...
      if (it != obj.end() && it->second.is_str())
        inst.name = it->second.as_str();
      auto it2 = obj.find("host");
      if (it2 != obj.end() && it2->second.is_str())
        inst.host = it2->second.as_str();
      auto it3 = obj.find("port");
      if (it3 != obj.end() && it3->second.is_num())
        inst.port = (int)it3->second.as_num();
      auto it4 = obj.find("pubkey");
      if (it4 != obj.end() && it4->second.is_str())
        inst.pubkey = it4->second.as_str();
      ...
      json::Object resp_o;
      resp_o["ok"] = true;
      resp_o["uuid"] = it_uuid->second.as_str();
  ...
  static void handle_heartbeat(const HttpRequest& req, const std::string& uuid, HttpResponse& resp)
  {
    if (!check_auth(req, resp))
      return;
    std::lock_guard<std::mutex> lk(g_instances_mu);
    ...
    if (it == g_instances.end()) {
      ...
      resp.body = R"({"error":"instance not found"})";
      return;
    }
  ...
  static void handle_deregister(const HttpRequest& req, const std::string& uuid, HttpResponse& resp)
  {
    if (!check_auth(req, resp))
      return;
    std::lock_guard<std::mutex> lk(g_instances_mu);
  ...
  static void handle_list_instances(HttpResponse& resp)
  {
    ...
    for (auto it = g_instances.begin(); it != g_instances.end();) {
      auto elapsed =
          std::chrono::duration_cast<std::chrono::seconds>(now - it->second.last_heartbeat).count();
    ...
    for (auto& [uuid, inst] : g_instances) {
      ...
      o["name"] = inst.name;
      o["host"] = inst.host;
      o["port"] = (double)inst.port;
      if (!inst.pubkey.empty())
        o["pubkey"] = inst.pubkey;
      auto elapsed =
          std::chrono::duration_cast<std::chrono::seconds>(now - inst.last_heartbeat).count();
  ...
  static void handle_request(const HttpRequest& req, HttpResponse& resp)
  {
    ...
    if (req.method == "PUT") {
      ...
      if (req.path.compare(0, p.size(), p) == 0)
        return handle_heartbeat(req, req.path.substr(p.size()), resp);
    }
    if (req.method == "DELETE") {
      std::string p = "/api/v1/rendezvous/instances/";
      if (req.path.compare(0, p.size(), p) == 0)
        return handle_deregister(req, req.path.substr(p.size()), resp);
    }
    ...
    if (req.method == "GET" && req.path == "/health") {
      json::Object o;
      o["ok"] = true;
      o["instances"] = (double)g_instances.size();
      resp.body = json::dumps(o);
      return;
    }
    resp.status_code = 404;
...
int main(int argc, char** argv)
{
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (std::string(argv[i]) == "--auth-token" && i + 1 < argc)
      g_auth_token = argv[++i];
    if (std::string(argv[i]) == "--help") {
      std::cerr << "WinInspect Rendezvous Server\nUsage: wininspect-rendezvous [--port <n>] [--auth-token <secret>]\n";
      return 0;
    }
  }
  if (!g_auth_token.empty())
    LOG_INFO("Auth enabled — registration requires Authorization: Bearer <token>");
  ...
  while (running.load()) {
    ...
    if (r > 0) {
      ...
      if (parse_http_request(std::string(buf), req)) {
        ...
        LOG_DEBUG("Rendezvous: " + req.method + " " + req.path + " -> " +
