...
UpdateInfo check_for_update(const std::string &current_version) {
  ...
  try {
    ...
    auto it_url = obj.find("html_url");
    if (it_url != obj.end() && it_url->second.is_str())
      info.download_url = it_url->second.as_str();
...
std::string download_update(const std::string &url, const std::string &version_tag) {
  ...
  std::wstring outFile = std::wstring(tempPath) + L"WinInspect-Installer-" +
                         std::wstring(version_tag.begin(), version_tag.end()) + L".exe";
  ...
  if (statusCode == 302 || statusCode == 301) {
    ...
    if (WinHttpQueryHeaders(hRequest, WINHTTP_QUERY_LOCATION,
      ...
      // Close and retry with redirect URL
      WinHttpCloseHandle(hRequest);
      WinHttpCloseHandle(hConnect);
      WinHttpCloseHandle(hSession);

      // Parse redirect URL
      std::wstring rurl(redirectUrl, redirectUrl + redirectSize / sizeof(wchar_t));
      // Simple: just re-download with new URL if it's an https URL
      if (rurl.rfind(L"https://", 0) == 0) {
        std::wstring rrest = rurl.substr(8);
        size_t rslash = rrest.find(L'/');
        std::wstring rhost, rpath;
        if (rslash == std::wstring::npos) {
          rhost = rrest;
          rpath = L"/";
        } else {
          rhost = rrest.substr(0, rslash);
          rpath = rrest.substr(rslash);
        }

        hSession = WinHttpOpen(L"WinInspect/1.0",
                                WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
                                WINHTTP_NO_PROXY_NAME,
                                WINHTTP_NO_PROXY_BYPASS, 0);
        if (!hSession) return {};

        hConnect = WinHttpConnect(hSession, rhost.c_str(),
                                   INTERNET_DEFAULT_HTTPS_PORT, 0);
        if (!hConnect) { WinHttpCloseHandle(hSession); return {}; }

        hRequest = WinHttpOpenRequest(hConnect, L"GET", rpath.c_str(), nullptr,
                                       WINHTTP_NO_REFERER,
        ...
        ok = WinHttpSendRequest(hRequest, headers, (DWORD)wcslen(headers),
                                 WINHTTP_NO_REQUEST_DATA, 0, 0, 0);
        if (!ok || !WinHttpReceiveResponse(hRequest, nullptr)) {
          WinHttpCloseHandle(hRequest);
          WinHttpCloseHandle(hConnect);
