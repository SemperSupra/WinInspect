...
FakeBackend::FakeBackend(std::vector<FakeWindow> windows) {
  for (auto& w : windows) w_.emplace(w.hwnd, std::move(w));
}
...
Snapshot FakeBackend::capture_snapshot() {
  ...
  for (const auto& [hwnd, w] : w_) {
    if (w.parent == 0) s.top.push_back(hwnd);
  }
...
std::vector<hwnd_u64> FakeBackend::list_children(const Snapshot&, hwnd_u64 parent) {
  ...
  for (const auto& [hwnd, w] : w_) if (w.parent == parent) out.push_back(hwnd);
...
std::optional<WindowInfo> FakeBackend::get_info(const Snapshot&, hwnd_u64 hwnd) {
  ...
  if (it == w_.end()) return std::nullopt;
...
std::optional<hwnd_u64> FakeBackend::pick_at_point(const Snapshot&, int, int, PickFlags) {
  ...
  if (s.top.empty()) return std::nullopt;
  return s.top.front();
}
