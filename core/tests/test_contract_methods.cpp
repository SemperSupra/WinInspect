...
DOCTEST_TEST_CASE("contract: window.getInfo") {
  ...
  auto r = core.handle(req, fb.capture_snapshot());
  CHECK(r.ok);
  CHECK(r.result.as_obj().at("title").as_str() == "Window A");
}
...
DOCTEST_TEST_CASE("contract: window.pickAtPoint") {
  ...
  auto r = core.handle(req, fb.capture_snapshot());
  CHECK(r.ok);
  CHECK(!r.result.as_obj().at("hwnd").as_str().empty());
}
...
DOCTEST_TEST_CASE("contract: screen.getPixel") {
  ...
  auto r = core.handle(req, fb.capture_snapshot());
  CHECK(r.ok);
  CHECK(r.result.as_obj().at("hex").as_str() == "#FF0000");
}
...
DOCTEST_TEST_CASE("contract: daemon.health") {
  ...
  auto r = core.handle(req, fb.capture_snapshot());
  CHECK(r.ok);
  CHECK(r.result.as_obj().at("os").as_str().size() > 0);
}
...
DOCTEST_TEST_CASE("contract: daemon.capabilities") {
  ...
  auto r = core.handle(req, fb.capture_snapshot());
  CHECK(r.ok);
  CHECK(r.result.as_obj().at("features").as_obj().at("uia").as_bool());
}
...
DOCTEST_TEST_CASE("contract: daemon.downloadUpdate") {
  ...
  auto r = core.handle(req, fb.capture_snapshot());
  CHECK(!r.ok); // fake backend returns empty path
}
...
DOCTEST_TEST_CASE("contract: update check returns structured data") {
  ...
  CHECK(obj.count("update_available") > 0);
  CHECK(obj.count("current_version") > 0);
}
...
DOCTEST_TEST_CASE("contract: snapshot capture + events lifecycle") {
  ...
  CHECK(resp_top.ok);
  CHECK(resp_top.result.as_arr().size() >= 1u);
...
DOCTEST_TEST_CASE("contract: bad method returns error") {
  ...
  auto r = core.handle(req, fb.capture_snapshot());
  CHECK(!r.ok);
  CHECK(r.error_code == "E_BAD_METHOD");
}
